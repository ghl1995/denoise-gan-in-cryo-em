...
def denoise(image):
    Basic_img = BM3D_1st_step(image)
    Final_img = BM3D_2nd_step(Basic_img, image)
    return Final_img


#############################
image = cv2.imread('D:\MSML\code\save_img\\betagan2SNR0.05\\noisy_image_150\str0_4746.png')
image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
print(image.shape)
image = image.reshape(369,369)
image = numpy.array(image)
print(image)
image = rescale_intensity(1.0 * image, out_range=(0, 1))
Basic_img = BM3D_1st_step(image)
Final_img = BM3D_2nd_step(Basic_img, image)
print(numpy.max(Basic_img))
...
plt.savefig('./bm3d.png', bbox_inches='tight', pad_inches=0.0)
# save denoised image

'''path = 'D:\cryo_em\code\denoise_code\denoise_gan\save_img\gan\model1'
path1 = path + '/noisy_image_150'
path2 = path + '/bm3d_denoise_image_150'
path3 = os.listdir(path1)
...
for filename in path3:
    ...
    #     print("success")
    Basic_img = BM3D_1st_step(image)
    # img1.append(Basic_img)

    # Basic_img = cv2.imread("Basic3.jpg", cv2.IMREAD_GRAYSCALE)

    Final_img = BM3D_2nd_step(Basic_img, image)
    scipy.misc.imsave(path2 + '/' + filename, Final_img)
    i = i + 1
    print(i)'''

'''image = cv2.imread('D:\cryo_em_data\EMDB2660_10028\ww_process_20180915_cryoSPARC\prepare_most_evident_images_for_xianyin\\raw_particles\\')
image = cv2.cvtColor(image, cv2.COLOR_BGR2GRAY)
...
Final_img = BM3D_2nd_step(Basic_img, image)
new_image = Final_img
sio.savemat('BM3D08.mat', {'new_image': new_image})'''

'''path = 'D:\cryo_em\code\\20180509_testing_denoising_algorithms_for_0_0_0\extracts_for_hanlin_\SNR0.1_\\test_dataset'
path1 = os.listdir(path)
for i in path1:
    data = mrcfile.open(os.path.join(path, i))
    data = data.data.reshape(128,128)
    data = rescale_intensity(1.0 * data, out_range=(0, 1))  # rescale the images
    print(data.shape)
    data = denoise(data)'''
