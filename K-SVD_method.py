...
mrc = mrcfile.open('./data/150test_SNR0.05_Gaussian.mrc').data
image = mrc[3]
...
Final_img, calc_time = denoising(image, dict_size=num_dict, sparsity=max_sparsity)
plt.axis('off')
frame = plt.gca()
# y 轴不可见
...
plt.savefig('./ksvd.png', bbox_inches='tight', pad_inches=0.0)
# save denoised image
...
path2 = path + '/ksvd_denoise_image_150'
...
for filename in path3:
    ...
    scipy.misc.imsave(path2 + '/' + filename, denoised_image)
    i = i + 1
    print(i)
